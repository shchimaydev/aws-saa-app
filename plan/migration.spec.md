...
- `login.tsx` — DONE: loader redirects already-authed users to `/quiz/1`; renders the ported auth modal (overlay + card, theme-styled) with `<LoginButton>`. Click → `signInWithGoogle()` (in `firebase.client.ts`, popup → `getIdToken()`) → `useFetcher` POST `{idToken}` to `/auth/session`. Errors surfaced via `fetcher.data.error` + popup-cancel handled quietly.
